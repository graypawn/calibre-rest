...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            ["--series-index", "4.5", "--title", "foobar"],
        ),
        (
            Book(identifiers={"isbn": "abcd", "asin": 1234}),
            ["--identifier", "isbn:abcd", "--identifier", "asin:1234"],
        ),
    ),
...
def test_handle_add_flags_simple(book, expected):
    ...
    assert got == ["calibredb", "add"] + expected
...
@pytest.mark.parametrize(
    "book, expected",
    (
        (Book(title="foobar", id=1234), ["--title", "foobar"]),
        (Book(pubdate="1234", size=1234), []),
    ),
...
@pytest.mark.parametrize(
    "book, expected",
    (
        (Book(title="foobar"), ["--field", "title:foobar"]),
        (Book(series_index=4.0), ["--field", "series_index:4.0"]),
    ),
)
def test_handle_update_flags(book, expected):
    cmd = ["calibredb", "set_metadata", "1"]
    got = dud_wrapper._handle_update_flags(cmd, book)
    assert got == ["calibredb", "set_metadata", "1"] + expected
